// Shared by server and client components (so it must not live in a "use client" module).
export const GRID_CLASSES =
  "grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 min-[1880px]:grid-cols-6";

/** Search results share the width with the map from `lg` up, so they use fewer columns. */
export const SEARCH_GRID_CLASSES = "grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 2xl:grid-cols-3";
